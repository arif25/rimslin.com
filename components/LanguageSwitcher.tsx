  ...
  return (
    <div className={`relative inline-block text-start max-w-full shrink-0 flex-shrink-0 ${className}`} ref={dropdownRef}>
      <button
        ...
        className="group inline-flex items-center justify-center gap-1 sm:gap-1.5 h-7 rounded-md md:rounded-lg border border-slate-200/80 bg-white/90 px-1.5 sm:px-2 md:px-2.5 py-0.5 text-[10px] sm:text-[11px] md:text-xs font-semibold leading-none text-slate-700 shadow-sm backdrop-blur-md transition-all hover:border-emerald-500 hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 dark:border-gulf-500/30 dark:bg-surface-100/90 dark:text-slate-200 dark:hover:border-gulf-400 dark:hover:bg-surface-200/90 dark:hover:text-white whitespace-nowrap shrink-0 flex-shrink-0"
        aria-expanded={isOpen}
        aria-haspopup="true"
        aria-label="Select Language"
      >
        <Globe className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-emerald-600 dark:text-gulf-400 transition-transform duration-300 group-hover:rotate-45 shrink-0 flex-shrink-0" />
        <span className="font-bold text-slate-800 dark:text-white tracking-wide truncate hidden sm:inline">{currentLang.label}</span>
        <span className="font-bold text-slate-800 dark:text-white tracking-wide truncate sm:hidden text-[10px] sm:text-[11px]">{currentLang.shortCode}</span>
        <ChevronDown
          className={`h-3 w-3 sm:h-3.5 sm:w-3.5 text-slate-400 transition-transform duration-200 shrink-0 flex-shrink-0 ${
      ...
      {isOpen && (
        <div
          className={`fixed sm:absolute ${
            isRTL ? "left-2 sm:left-0 origin-top-left" : "right-2 sm:right-0 origin-top-right"
          } top-10 sm:top-full mt-1.5 w-44 sm:w-48 max-w-[calc(100vw-16px)] bg-white border border-slate-200 rounded-xl shadow-xl z-[70] p-1.5 dark:bg-[#08150d] dark:border-gulf-500/30 dark:shadow-2xl dark:ring-1 dark:ring-white/10`}
        >
